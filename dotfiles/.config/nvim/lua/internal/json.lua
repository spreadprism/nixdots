...
do
			...
			elseif c == "\\" then
				...
				if esc == "u" then
					...
					elseif n >= 0xDC00 and n < 0xE000 then
						n, surrogate_pair_started = surrogate_pair_started
							and (surrogate_pair_started - 0xD800) * 0x400 + (n - 0xDC00) + 0x10000
					end
					if n then
						...
						elseif n < 0x10000 then
							...
							x = string_char(
								0xE0 + (math_floor(n / 64 / 64) % 0x10),
								0x80 + (math_floor(n / 64) % 0x40),
								0x80 + (n % 0x40)
							)
						else
							-- [1111 0xxx] [10xx xxxx] [10xx xxxx] [10xx xxxx]
							x = string_char(
								0xF0 + (math_floor(n / 64 / 64 / 64) % 8),
								0x80 + (math_floor(n / 64 / 64) % 0x40),
								0x80 + (math_floor(n / 64) % 0x40),
								0x80 + (n % 0x40)
							)
						end
	...
	function isConvertibleToString(o)
		local t = type(o)
		return t == "string"
			or t == "number" and isRegularNumber(o) and (math_type(o) == "integer" or math_floor(o) == o)
	end

	local is_Inf_or_NaN =
		{ [tostring(1 / 0)] = true, [tostring(-1 / 0)] = true, [tostring(0 / 0)] = true, [tostring(-(0 / 0))] = true }
